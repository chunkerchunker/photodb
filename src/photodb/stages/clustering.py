...
class ClusteringStage(BaseStage):
    ...
    def __init__(self, repository, config: dict):
        ...
        logger.debug(f"Initialized ClusteringStage with threshold {self.clustering_threshold}")
    ...
    def _cluster_single_face(self, face: dict) -> None:
        ...
        if not isinstance(embedding, np.ndarray):
            try:
                ...
                logger.error(
                    f"Failed to convert embedding to numpy array for face {face_id}: {e}, type: {type(embedding)}"
                )
                return
        ...
        try:
            ...
            if embedding_dim != 512:
                logger.error(
                    f"Invalid embedding dimension for face {face_id}: {embedding_dim} (expected 512)"
                )
                return
        except TypeError as e:
            logger.error(
                f"Cannot get length of embedding for face {face_id}: {e}, embedding type: {type(embedding)}"
            )
            return
    ...
    def _create_new_cluster(self, face_id: int, embedding: np.ndarray) -> None:
        ...
        cluster_id = self.repository.create_cluster(
            centroid=embedding, representative_face_id=face_id, medoid_face_id=face_id, face_count=1
        )

        # Assign face to cluster
        self.repository.update_face_cluster(
            face_id=face_id, cluster_id=cluster_id, cluster_confidence=1.0, cluster_status="auto"
        )

        logger.debug(f"Created new cluster {cluster_id} for face {face_id}")

    def _assign_to_cluster(
        self, face_id: int, cluster_id: int, confidence: float, embedding: np.ndarray
    ) -> None:
        ...
        with self.repository.get_connection() as conn:
            with conn.cursor() as cur:
                ...
                conn.commit()
                logger.debug(
                    f"Assigned face {face_id} to cluster {cluster_id} with confidence {confidence:.3f}"
                )
    ...
    def _mark_for_review(self, face_id: int, matching_clusters: List[Tuple[int, float]]) -> None:
        ...
        candidates = [
            (face_id, cluster_id, 1.0 - distance) for cluster_id, distance in matching_clusters
        ]
        ...
        logger.debug(
            f"Face {face_id} marked for review with {len(matching_clusters)} potential clusters"
        )
