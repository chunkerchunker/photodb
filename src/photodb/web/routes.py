...
def month_view(year, month):
    ...
    for photo in photos:
        ...
        if photo.get('description'):
            photo['short_description'] = (photo['description'][:47] + '...') if len(photo['description']) > 50 else photo['description']
        else:
    ...
    if photo.get('metadata_extra'):
        try:
            if isinstance(photo['metadata_extra'], str):
                photo['metadata_formatted'] = json.dumps(json.loads(photo['metadata_extra']), indent=2)
            else:
