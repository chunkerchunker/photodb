...
export function meta() {
  return [{ title: "Storyteller - Collections" }, { name: "description", content: "Manage your photo collections" }];
}
...
export default function CollectionsPage({ loaderData }: Route.ComponentProps) {
  ...
  const handleSelectCollection = (collectionId: number) => {
    if (collectionId !== currentCollectionId) {
      fetcher.submit({ collectionId: collectionId.toString() }, { method: "post", action: "/api/collections/switch" });
    }
  };
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="container mx-auto px-4 py-6">
        <div className="space-y-6">
          ...
          <p className="text-gray-600">Select which collection to view. Your current collection is highlighted.</p>

          {collections.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {collections.map((collection) => {
                const isActive = collection.id === currentCollectionId;
                const hasAvatar = !!collection.avatar_detection_id;
                  ...
                  >
                    <Card
                      className={`hover:shadow-lg transition-all h-full cursor-pointer ${
                        isActive ? "ring-2 ring-blue-500 ring-offset-2 bg-blue-50" : "hover:ring-2 hover:ring-gray-300"
                      }`}
                    >
                        ...
                        <div className="flex items-start space-x-4">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center space-x-2">
                              ...
                              {isActive && <Check className="h-5 w-5 text-blue-500 flex-shrink-0" />}
                            </div>
            ...
            <div className="text-center py-12">
              ...
              <div className="text-gray-400 text-sm mt-2">You are not a member of any collections yet.</div>
            </div>
          )}
        </div>
      </main>
