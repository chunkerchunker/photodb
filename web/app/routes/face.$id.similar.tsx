...
export default function SimilarFacesPage({ loaderData }: Route.ComponentProps) {
	...
	const unclusteredFaces = similarFaces.filter(
		(f: SimilarFace) => !f.cluster_id,
	);
	...
	const searchClusters = useCallback(async (query: string) => {
		...
		try {
			const response = await fetch(
				`/api/clusters/search?q=${encodeURIComponent(query)}`,
			);
	...
	const selectAll = () => {
		setSelectedFaces(
			unclusteredFaces.map((f: SimilarFace) => parseInt(f.id, 10)),
		);
	};
						...
						<div className="flex items-center space-x-4">
							...
							<div>
								...
								<p className="text-xs text-gray-400 mt-1">
									Face size: {Math.round(face.bbox_width)}×
									{Math.round(face.bbox_height)}px
								</p>
							</div>
						</div>
					</CardContent>
					...
					<div className="flex items-center justify-between">
						<div className="space-y-1">
							...
							<p className="text-xs text-gray-500">
								Select similar faces to group with the source face into a new or
								existing cluster.
							</p>
						</div>
						<div className="flex items-center space-x-2">
								...
								<>
									<div className="flex items-center space-x-2 mr-4">
										<Checkbox
											id="select-all"
											checked={
												selectedFaces.length === unclusteredFaces.length &&
												unclusteredFaces.length > 0
											}
							...
							<Dialog
								open={addToClusterModalOpen}
								onOpenChange={setAddToClusterModalOpen}
							>
								...
								<DialogContent className="max-w-lg">
									<DialogHeader>
										...
										<DialogDescription>
											Search for a cluster to add {selectedFaces.length + 1}{" "}
											face
							...
							>
								...
								{isSubmitting
									? "Creating..."
									: `Create Cluster (${selectedFaces.length + 1})`}
							</Button>
						</div>
					</div>
				...
				{similarFaces.length > 0 ? (
										...
										<div className="text-center space-y-3">
											<div className="relative w-32 h-32 mx-auto bg-gray-100 rounded-lg border overflow-hidden">
												<Link
													to={
														isClustered
															? `/cluster/${similarFace.cluster_id}`
															: `/photo/${similarFace.photo_id}`
													}
											...
											<div className="space-y-1">
												...
												<div className="text-xs text-gray-400">
													{Math.round(similarFace.bbox_width)}×
													{Math.round(similarFace.bbox_height)}px
												</div>
												{isClustered ? (
													<Link
														to={`/cluster/${similarFace.cluster_id}`}
														onClick={(e) => e.stopPropagation()}
													>
														<Badge
															variant="secondary"
												...
												{(similarFace.age_estimate || similarFace.gender) && (
													<div className="flex items-center justify-center space-x-1 mt-1">
														{formatAge(similarFace.age_estimate) && (
															<Badge
																variant="outline"
																className="text-xs bg-blue-50 border-blue-200 px-1.5 py-0"
															>
					...
					<div className="text-center py-12">
						...
						<div className="text-gray-500 text-lg">
							No similar faces found above {Math.round(threshold * 100)}%
						</div>
