...
function ThreeWall({ photos, year, month, totalPhotos, monthName }: ThreeWallProps) {
  ...
  const updateTileCurve = useCallback(() => {
    ...
    tilesRef.current.forEach((tile) => {
      ...
      // Apply curve: rotate around Y axis and offset Z based on distance from center
      const normalizedX = worldX / (cameraZRef.current.z * 1.5);
      const rotationY = Math.max(
        -Math.PI / 4,
        Math.min(Math.PI / 4, normalizedX * CURVE_FACTOR * cameraZRef.current.z),
      );
      const offsetZ = -Math.abs(normalizedX) * Z_OFFSET_FACTOR * cameraZRef.current.z;
      ...
      tile.reflectionMesh.position.x = tile.baseX;
