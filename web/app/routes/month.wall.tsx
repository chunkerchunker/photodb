...
function ThreeWall({
			...
			if (isVisible && !loadedTexturesRef.current.has(tile.photo.id)) {
				...
				textureLoaderRef.current?.load(
					...
					(texture) => {
						...
						const reflectionMaterial = tile.reflectionMesh
	...
	const updateTileCurve = useCallback(() => {
		...
		tilesRef.current.forEach((tile) => {
			...
			tile.shadowMesh.position.x =
				curveX - wallPositionRef.current.x + shadowOffsetX;
	...
	const animate = useCallback(() => {
		...
		if (transition.active) {
			...
			cameraRef.current.position.x =
				transition.startX +
				(transition.targetX - transition.startX) * easedProgress;
			cameraRef.current.position.y =
				transition.startY +
				(transition.targetY - transition.startY) * easedProgress;
			cameraRef.current.position.z =
				transition.startZ +
				(transition.targetZ - transition.startZ) * easedProgress;

			// Interpolate wall position
			wallPositionRef.current.x =
				transition.startWallX +
				(transition.targetWallX - transition.startWallX) * easedProgress;
			...
			if (transition.direction === "in") {
				...
				if (transition.progress > fadeStart) {
					const fadeProgress =
						(transition.progress - fadeStart) / (1 - fadeStart);
				...
				if (transition.progress >= 1) {
					...
					if (transition.targetPhoto) {
						...
						navigate(`/photo/${transition.targetPhoto.id}`);
					}
				}
			} else {
			...
			if (Math.abs(pos) > maxX - softZone) {
				...
				const distanceIntoSoftZone =
					(Math.abs(pos) - (maxX - softZone)) / softZone;
	...
	const handleMouseMove = useCallback((e: MouseEvent) => {
		...
		if (isDraggingRef.current) {
			...
			flickVelocityRef.current =
				(deltaX / deltaTime) * FLICK_VELOCITY_MULTIPLIER;
			...
			const maxX = wallWidthRef.current / 2 - 2;
			wallPositionRef.current.x = Math.max(
				-maxX,
				Math.min(maxX, wallPositionRef.current.x),
			);
		...
		const intersects = raycaster.intersectObjects(
			wallContainerRef.current?.children || [],
			false,
		);

		const hit = intersects.find(
			(i) => i.object.userData.photo && !i.object.userData.isReflection,
		);
		if (hit) {
	...
	const handleWheel = useCallback((e: WheelEvent) => {
		...
		if (Math.abs(e.deltaX) > Math.abs(e.deltaY)) {
			...
			cameraZRef.current.targetZ = Math.max(
				CAMERA_Z_MIN,
				Math.min(CAMERA_Z_MAX, cameraZRef.current.targetZ),
			);
		}
	}, []);
	...
	useEffect(() => {
		...
		photos.forEach((photo, index) => {
			...
			const x =
				column * (TILE_WIDTH + TILE_GAP) - wallWidth / 2 + TILE_WIDTH / 2;
			...
			const shadowGeometry = new THREE.PlaneGeometry(
				TILE_WIDTH + SHADOW_BLUR * 4,
				TILE_HEIGHT + SHADOW_BLUR * 4,
			);
	...
	useEffect(() => {
		...
		const handleTouchMove = (e: TouchEvent) => {
			if (e.touches.length === 2) {
				...
				cameraZRef.current.targetZ = Math.max(
					CAMERA_Z_MIN,
					Math.min(CAMERA_Z_MAX, initialZoom * scale),
				);
			}
		};
